...
双击 `Codex安装器.exe` 即可，**不需要 Node.js、不需要浏览器、不需要任何运行库**，拷到别的 Windows 电脑上照样能跑。
...
双击 `Codex安装器.exe` 打开界面，默认自动检查更新。界面上的按钮：
...
```text
Codex安装器.exe                 打开界面并自动检查更新
Codex安装器.exe --update        自动完成 检查 → 下载 → 安装
Codex安装器.exe --check         只检查
Codex安装器.exe --download      只下载
Codex安装器.exe --install-local 直接安装本地已有的包
Codex安装器.exe --help          显示帮助
```
...
**要装到别的电脑上**
把 `Codex安装器.exe` 单独拷过去就行，不需要带任何其它文件。要求 Windows 10 版本 2004（内部版本 19041）及以上——这也是 Codex 本身的要求。
...
```powershell
.\Codex安装器.exe --render-ui out.png 1.5   # 模拟 150% 缩放
```
